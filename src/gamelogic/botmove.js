var move = 0;
let arr = ['o', 'x'];
export default function botmove(x, grids, size, type, level) {
//  const mark = x==='x'?'o':'x';
            ...
            for(let n=1; n<l+1; n++) {
              if((l===3||l===2) && m) {
                ...
                for(let w = 1; w < 4-q; w++) {
                  ...
                  if(i <size-1 && j>0 && i > l && j < size-l-1 && grids[i][j]===arr[k] && n!==w && grids[i][j] === grids[i-n][j+n]) {
                    streak.diag21++;
                    if(streak.diag21 === l) {
                      streak.diag21=0;
                      if(grids[i-w][j+w]==='') return {i:i-w, j:j+w};
              ...
              if(l > 1) {
                if(j>0 && j<size-l-1 && grids[i][j]===arr[k] && grids[i][j+n-1] === grids[i][j+n]) {
                  ...
                  if(streak.hori === l) {
                    ...
                    else if(grids[i][j-1]==='') return {i:i, j:j-1};
                  }
                }
                if(i>0 && i < size-l-1 && grids[i][j]===arr[k] && grids[i+n-1][j] === grids[i+n][j]) {
                  ...
                  if(streak.verti === l) {
                    ...
                    else if(grids[i-1][j]==='') return {i:i-1, j:j};
                  }
                }
                if(i>0 && j>0 && i < size-l-1 && j < size-l-1 && grids[i][j]===arr[k] && grids[i+n-1][j+n-1] === grids[i+n][j+n]) {
                  ...
                  if(streak.diag1 === l) {
                    ...
                    else if(grids[i-1][j-1]==='') return {i:i-1, j:j-1};
                  }
                }
